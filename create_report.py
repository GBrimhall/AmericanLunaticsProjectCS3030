...
def convert_date():
    ...
    if verify(beg_date) and verify(end_date):
        bDate = beg_date[:4] + '-' + beg_date[4:6] + '-' + beg_date[6:]
        + '00:00:00'
...
def query():
    ...
    for row in c.execute("SELECT trans_id, trans_date, card_num, qty, amt, prod_desc, total FROM trans LEFT JOIN trans_line USING (trans_id) LEFT JOIN products USING (prod_num) WHERE trans_date > '" + bDate + "' AND trans_date < '" + eDate + "'"):
        ...
        if transID != row[0]:
            ...
            if report == "":
                ...
                report += '{s:{c}>{n}}'.format(s=str('{:.2f}'.format(total))
                                               .replace('.', ''), n=6, c='0')
            ...
            if row[4] is None:
                ...
                price = '{s:{c}>{n}}'.format(s=str(price).replace('.', ''),
                                             n=6, c='0')
    ...
    text_file = open('company_trans_' + sys.argv[1] + '_' + sys.argv[2] +
                     '.dat', 'w')
...
def zip_file():
    # Zip file
    with zipfile.ZipFile('company_trans_' + sys.argv[1] + '_' +
                         sys.argv[2] + '.dat.zip', 'w') as myzip:
